...
extern inline simd_float3 BezierVertexMake(BCCubic cubic, uint8_t vertexID, uint8_t vertexesPerCubic, simd_float3x3 transform);
...
__attribute__((const))
__attribute__((overloadable))
simd_float3 MakeBezierVertexAdjusting(BCCubic cubic, uint8_t vertexID, uint8_t vertexesPerCubic, simd_float3x3 transform, bc_float_t minimum, bc_float_t maximum, bc_float_t minimumDeltaT) {
    __BC_ASSERT(minimum <= maximum);
    if (maximum - minimum < minimumDeltaT) {
        minimum = simd_max(0.0f,minimum - minimumDeltaT);
        maximum = simd_min(1.0f,maximum + minimumDeltaT);
    }
    return BezierVertexMake(cubic, vertexID, vertexesPerCubic, transform, minimum, maximum);
}
